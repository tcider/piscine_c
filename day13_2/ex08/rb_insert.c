...
t_rb_node	*rb_sm_rotate(t_rb_node *node, t_rb_node *parent, t_rb_node *grand)
{
...
t_rb_node	*rb_bg_rotate(t_rb_node *node, t_rb_node *parent, t_rb_node *grand)
{
...
t_rb_node	*rb_balance(t_rb_node *node, t_rb_node *parent)
{
	t_rb_node	*grand;
	t_rb_node	*uncle;
	...
	{
		node = rb_sm_rotate(node, parent, grand);
		parent = node->parent;
		grand = parent->parent;
		parent = rb_bg_rotate(node, parent, grand);
		parent->color = RB_BLACK;
		if (!parent->parent)
			return (parent);
	}
	...
	int (*cmpf)(void *, void *))
{
	...
	if ((node->parent)->color == RB_RED)
		tmp = rb_balance(node, node->parent);
	if (tmp)
		*root = tmp;
}
